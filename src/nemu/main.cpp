...
int main(int argc, char** argv) {
    ...
    NEMU_LOG_INFO("Init", "  Version 0.2.0 (Milestone 8 - Full Subsystems)          ");
    NEMU_LOG_INFO("Init", "=========================================================");

    // 1. Initialize Virtual File System (VFS)
    filesystem::VirtualFileSystem vfs;
    std::filesystem::path sdmc_root = std::filesystem::current_path() / "sdmc";
    std::filesystem::path save_root = std::filesystem::current_path() / "save";
    vfs.Mount("sdmc:/", sdmc_root, false);
    vfs.Mount("save:/", save_root, false);
    NEMU_LOG_INFO("VFS", "Mounted SD Card at '{}'", sdmc_root.string());

    // 2. Initialize Direct3D 12 / Null GPU Backend
    auto gpu_backend = gpu::GpuFactory::CreateBackend(1280, 720);
    NEMU_LOG_INFO("GPU", "Graphics Engine initialized: {}", gpu_backend->GetBackendName());
    gpu::Maxwell3D maxwell(gpu_backend);

    // 3. Initialize XAudio2 / Null Audio Backend
    auto audio_backend = audio::AudioFactory::CreateBackend(48000, 2);
    audio_backend->Start();
    NEMU_LOG_INFO("Audio", "Sound Engine initialized: {}", audio_backend->GetBackendName());

    // 4. Initialize Input / HID System
    hid::HidManager hid_manager;
    NEMU_LOG_INFO("HID", "Xbox Wireless Controller input mapper initialized (8 players supported)");

    // 5. Initialize Horizon Kernel Process and Address Space
    ...
    // Check if an NRO binary was specified on the command line
    if (argc > 1) {
        const std::string_view nro_path = argv[1];
        NEMU_LOG_INFO("Loader", "Loading homebrew executable: {}", nro_path);
        auto loaded = loader::NroLoader::LoadFromFile(nro_path, memory);
        if (loaded) {
            entry_point = loaded->entry_point;
            NEMU_LOG_INFO("Loader", "Loaded NRO successfully at entry point 0x{:016X}", entry_point);
        } else {
            NEMU_LOG_ERROR("Loader", "Failed to load NRO from '{}'. Falling back to internal demo.", nro_path);
        }
    }

    // If no external NRO loaded, synthesize a self-contained homebrew binary
    if (entry_point == 0) {
        NEMU_LOG_INFO("Init", "No external NRO provided; synthesizing valid NRO homebrew binary in memory...");
    ...
    // 6. Create Main Thread and Initialize Context
    ...
    // 7. Configure Supervisor Call (SVC) Dispatcher
    ...
    // 8. Render a startup frame via GPU
    ...
    NEMU_LOG_INFO("CPU", "Executing guest homebrew thread at PC 0x{:016X}...", cpu.pc);
