...
int main() {
    ...
    // 5b. Realistic prod.keys + title.keys format round-trip
    {
        using namespace nemu::core::crypto;
        // Representative prod.keys text as produced by standard key dumps
        // (lines are `name = hex`; master/header/kak/titlekek + per-title keys).
        const std::string prod_keys =
            "# equivalent-production-keys generated by a key dump tool\n"
            "header_key = 00112233445566778899aabbccddeeff00112233445566778899aabbccddeeff\n"
            "kermit_master_key_00 = 11111111111111111111111111111111\n"
            "key_area_key_application_00 = 22222222222222222222222222222222\n"
            "key_area_key_application_01 = 33333333333333333333333333333333\n"
            "titlekek_00 = 44444444444444444444444444444444\n"
            "titlekek_01 = 55555555555555555555555555555555\n"
            // 32-hex rights id: "0100" + 24 x 'a' + "0000".
            "0100aaaaaaaaaaaaaaaaaaaaaaaa0000 = 66666666666666666666666666666666\n";

        const std::string title_keys =
            "# title.keys: rights-id = title key\n"
            "0100cccccccccccccccccccccccc0000 = 77777777777777777777777777777777\n";

        KeyStore ks;
        NEMU_TEST_ASSERT(ks.LoadFromText(prod_keys), "prod.keys parses");
        NEMU_TEST_ASSERT(ks.LoadFromText(title_keys), "title.keys parses");

        auto kc = ks.GetKeyCompleteness();
        NEMU_TEST_ASSERT(kc.has_header_key, "header_key present");
        NEMU_TEST_ASSERT(kc.has_master_key, "kermit master key recognized");
        NEMU_TEST_ASSERT(kc.has_key_area_key, "key area key present");
        NEMU_TEST_ASSERT(kc.has_titlekek, "titlekek present");
        NEMU_TEST_ASSERT(kc.title_key_count == 2, "two title keys recognized (prod + title files)");

        // Key area key by generation.
        auto kak0 = ks.GetKeyAreaKey(0, 0);
        auto kak1 = ks.GetKeyAreaKey(1, 0);
        NEMU_TEST_ASSERT(kak0 && kak0->size() == 16, "KAK gen 0");
        NEMU_TEST_ASSERT(kak1 && kak1->size() == 16, "KAK gen 1");

        // Title keys resolved by their rights ids (bare hex names).
        auto tk_prod = ks.GetTitleKey("0100aaaaaaaaaaaaaaaaaaaaaaaa0000");
        auto tk_title = ks.GetTitleKey("0100cccccccccccccccccccccccc0000");
        NEMU_TEST_ASSERT(tk_prod.has_value(), "prod.keys entry resolves");
        NEMU_TEST_ASSERT(tk_title.has_value(), "title.keys entry resolves");

        std::cout << "  - Realistic prod.keys/title.keys format round-trip: PASSED" << std::endl;
    }
